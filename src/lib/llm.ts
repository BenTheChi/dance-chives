...
export async function parsePlaylistWithGroq(
  ...
  try {
    ...
    // Step 1: Categorize videos into sections (Groq)
    const categorizationPrompt = buildCategorizationPrompt(videos, context);
    const categorizedResponse = await callGroqAPI(
    ...
    for (let i = 0; i < fixedResponse.sections.length; i++) {
      ...
      if (
        ...
        console.log(JSON.stringify(bracketResponse, null, 2));
    ...
    throw new Error(`Failed to parse playlist with LLMs: ${String(error)}`);
  }
}
