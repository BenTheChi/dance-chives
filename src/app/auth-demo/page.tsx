...
export default function AuthDemoPage() {
  ...
  const isModerator = userAuthLevel >= AUTH_LEVELS.REGIONAL_MODERATOR;
  const isCreator = userAuthLevel >= AUTH_LEVELS.REGIONAL_CREATOR;
    ...
    <div className="container mx-auto py-8 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <Badge
              variant={
                userAuthLevel >= AUTH_LEVELS.REGIONAL_CREATOR
                  ? "default"
                  : "secondary"
              }
            >
                ...
                <div className="border-t pt-3">
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    {userAuthLevel === AUTH_LEVELS.BASE_USER && (
                      <li>• Tag yourself in events (with approval)</li>
                    )}
                    {isCreator && (
                      <>
                        <li>
                          • Create events in{" "}
                          {userAuthLevel >= AUTH_LEVELS.GLOBAL_CREATOR
                            ? "any city"
                            : "restricted cities"}
                        </li>
                    ...
                    {isModerator && (
                      <>
                        <li>
                          • Update/delete any events in{" "}
                          {userAuthLevel >= AUTH_LEVELS.GLOBAL_MODERATOR
                            ? "any city"
                            : "restricted cities"}
                        </li>
                        <li>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>How to use auth levels in your code</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  if (canUpdateEvent(session?.user?.auth || 0, context, session?.user?.id || "")) {
    ...
    throw new Error("Permission denied");
            ...
            <div>
              ...
              <pre className="bg-muted p-3 rounded text-sm overflow-x-auto">
                {`import { useSession } from "next-auth/react";
import { hasAuthLevel, AUTH_LEVELS, canCreateEventsInAnyCity } from "@/lib/utils/auth-utils";
