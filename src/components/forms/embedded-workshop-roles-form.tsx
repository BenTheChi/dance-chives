...
interface EmbeddedWorkshopRolesFormProps {
  control: Control<any>;
...
export default function EmbeddedWorkshopRolesForm({
  control,
  setValue,
  roles,
  workshopIndex,
}: EmbeddedWorkshopRolesFormProps) {
  ...
  const addRole = () => {
    ...
    // Add to our local state array using nested path
    setValue(`workshops.${workshopIndex}.roles`, [...roles, newRole]);
  };

  // Remove a role entry
  const removeRole = (idToRemove: string) => {
    setValue(
      `workshops.${workshopIndex}.roles`,
      ...
      {roles.map((role, index) => (
        <div className="flex items-end gap-5 mb-4" key={role.id}>
          ...
          <div className="flex gap-5 w-full">
            <FormField
              control={control}
              name={`workshops.${workshopIndex}.roles.${index}.title`}
              render={() => (
                <FormItem className="w-full">
                  ...
                  <FormControl>
                    <Select
                      onValueChange={(value) => {
                        setValue(
                          `workshops.${workshopIndex}.roles.${index}.title`,
                          value
                        );
                      }}
            ...
            <DebouncedSearchSelect<UserSearchItem>
              control={control}
              name={`workshops.${workshopIndex}.roles.${index}.user`}
              ...
              onChange={(value) => {
                setValue(
                  `workshops.${workshopIndex}.roles.${index}.user`,
                  value as UserSearchItem | null
                );
              }}
